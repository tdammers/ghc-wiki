...
<table><tr><th>[\#9980](http://gitlabghc.nibbler/ghc/ghc/issues/9980)</th>
<td>TcS monad is too heavy</td></tr>
<tr><th>[\#10077](http://gitlabghc.nibbler/ghc/ghc/issues/10077)</th>
<td>Providing type checker plugin on command line results in false cyclic import error</td></tr>
<tr><th>[\#11435](http://gitlabghc.nibbler/ghc/ghc/issues/11435)</th>
<td>Evidence from TC Plugin triggers core-lint warning</td></tr>
<tr><th>[\#11457](http://gitlabghc.nibbler/ghc/ghc/issues/11457)</th>
<td>Run type-checker plugins before GHC's solver</td></tr>
<tr><th>[\#12780](http://gitlabghc.nibbler/ghc/ghc/issues/12780)</th>
<td>Calling "do nothing" type checker plugin affects type checking when it shouldn't</td></tr>
<tr><th>[\#15147](http://gitlabghc.nibbler/ghc/ghc/issues/15147)</th>
<td>Type checker plugin receives Wanteds that are not completely unflattened</td></tr>
<tr><th>[\#15248](http://gitlabghc.nibbler/ghc/ghc/issues/15248)</th>
...
<tr><th>[\#15745](http://gitlabghc.nibbler/ghc/ghc/issues/15745)</th>
...
<table><tr><th>[\#9840](http://gitlabghc.nibbler/ghc/ghc/issues/9840)</th>
<td>Permit empty closed type families</td></tr>
<tr><th>[\#10078](http://gitlabghc.nibbler/ghc/ghc/issues/10078)</th>
<td>tcPluginStop of a type checker plugin is not called if an error occurs</td></tr>
<tr><th>[\#11462](http://gitlabghc.nibbler/ghc/ghc/issues/11462)</th>
<td>Use of typechecker plugin erroneously triggers "unbound implicit parameter" error</td></tr>
<tr><th>[\#11525](http://gitlabghc.nibbler/ghc/ghc/issues/11525)</th>
<td>Using a dummy typechecker plugin causes an ambiguity check error</td></tr>
<tr><th>[\#14691](http://gitlabghc.nibbler/ghc/ghc/issues/14691)</th>
<td>Replace EvTerm with CoreExpr</td></tr>
<tr><th>[\#15633](http://gitlabghc.nibbler/ghc/ghc/issues/15633)</th>
...
Plugins that define type families often need to ensure that those type families have no normal instances, to avoid inconsistency, but empty closed type families were previously rejected ([\#9840](http://gitlabghc.nibbler/ghc/ghc/issues/9840)). They are now permitted in HEAD. See [
...
**Richard:** This makes me uncomfortable, in exactly the same way that I was made to feel uncomfortable in the comments starting with [comment:4:ticket:9840](http://gitlabghc.nibbler/ghc/ghc/issues/9840). The fact that the new, (what I will call) *external* type families will behave differently than internal type families is further evidence that something is amiss. (The difference in behavior I'm referring to is the difference between `matchFam` and `reduceTyFamApp_maybe`.) This, of course, ties into [\#9636](http://gitlabghc.nibbler/ghc/ghc/issues/9636) as well and to some of the more esoteric issues that cropped up while working on [\#6018](http://gitlabghc.nibbler/ghc/ghc/issues/6018)/[
Phab:D202](https://phabricator.haskell.org/D202) and perhaps even [\#10327](http://gitlabghc.nibbler/ghc/ghc/issues/10327). I would love to approach this problem with the idea of making broader changes instead of looking for a minimal change just to support typechecker plugins better. **End Richard**
...
This is now implemented; see [\#14691](http://gitlabghc.nibbler/ghc/ghc/issues/14691).
...
- making particular type families [injective](injective-type-families) (cf. [\#6018](http://gitlabghc.nibbler/ghc/ghc/issues/6018))

- extra improvement for closed type families (cf. [\#10227](http://gitlabghc.nibbler/ghc/ghc/issues/10227))

- adding functional dependency style behaviour to particular typeclasses

- eta-expansion of type-level products ([\#7259](http://gitlabghc.nibbler/ghc/ghc/issues/7259))
...
See [\#10077](http://gitlabghc.nibbler/ghc/ghc/issues/10077) for more information.
