...
- **Converting between Int and Integer**.  It's quite commonly the case that, after some inlining, we get something like `integerToInt (intToInteger i)`, which converts an `Int` to an `Integer` and back.  This *must* optimise away (see [\#5767](http://gitlabghc.nibbler/ghc/ghc/issues/5767)).  We do this by requiring that the `integer` package exposes
