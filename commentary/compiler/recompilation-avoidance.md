...
(Correctly triggering recompilation when packages change was one of the things we fixed when implementing fingerprints, see [\#1372](http://gitlabghc.nibbler/ghc/ghc/issues/1372)).
...
- There are still some cases where an interface can change without changing the source code.  The ones we know about are listed in [\#4012](http://gitlabghc.nibbler/ghc/ghc/issues/4012)
