...
However, this does not bring `fullName` into scope as a field, [as previously observed](records/overloaded-record-fields/design#). Moreover, it is difficult to check the type family instances for consistency. For example, given the following declaration
