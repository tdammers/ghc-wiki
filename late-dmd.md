...
The -flate-dmd-anal flag runs the demand analysis a second time just before CorePrep, with a subsequent run of the Simplifier.  Cf [\#7782](http://gitlabghc.nibbler/ghc/ghc/issues/7782).



It's not on by default yet, but we hope -O2 will eventually imply it, perhaps even for the GHC 7.8 release. UPDATE (2018-02-21): nofib results with `-flate-dmd-anal` were not conclusive, see [\#6087](http://gitlabghc.nibbler/ghc/ghc/issues/6087) for results.
...
There are some tickets documenting runtime bugs that can be cleaned up by running the demand analyzer (followed by a simplifier run) a second time at the end of the pipeline: [\#4941](http://gitlabghc.nibbler/ghc/ghc/issues/4941), [\#5302](http://gitlabghc.nibbler/ghc/ghc/issues/5302), [\#6087](http://gitlabghc.nibbler/ghc/ghc/issues/6087). [\#6070](http://gitlabghc.nibbler/ghc/ghc/issues/6070) ? Others?
...
For cryptarithm2, (cf remarks in [\#4941](http://gitlabghc.nibbler/ghc/ghc/issues/4941))


- 4% increase allocation is due to reboxing
- 4% is due to dead closures, because the fix in [\#4962](http://gitlabghc.nibbler/ghc/ghc/issues/4962) isn't working for some reason.
