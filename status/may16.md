...
- A new **DeriveLift language extension**, allowing the `Lift` type class from `Language.Haskell.TH.Syntax` to be derived automatically. This was introduced in the spirit of `DeriveDataTypeable` and `DeriveGeneric` to allow easier metaprogramming, and to allow users to easily define `Lift` instances without needing to depend on the existence of Template Haskell itself (see [\#1830](http://gitlabghc.nibbler/ghc/ghc/issues/1830)).
...
- Significant improvements in cross-platform support, including a variety of fixes to **Windows linker support** ([
  ...
  Phab:D1805](https://phabricator.haskell.org/D1805)), great improvements in **reliability on ARM** ([\#11206](http://gitlabghc.nibbler/ghc/ghc/issues/11206)), revived **unregisterised [
...
- Improved support for **pattern synonyms**, including record syntax ([\#8582](http://gitlabghc.nibbler/ghc/ghc/issues/8582)) and the ability to associate pattern synonyms with type constructors on export, implemented by Matthew Pickering. See Matthew's [
