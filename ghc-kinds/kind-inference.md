...
Though originally written with respect to [\#9200](http://gitlabghc.nibbler/ghc/ghc/issues/9200), there are more issues at play here. Relevant other tickets: [\#9201](http://gitlabghc.nibbler/ghc/ghc/issues/9201), [\#9427](http://gitlabghc.nibbler/ghc/ghc/issues/9427), [\#14451](http://gitlabghc.nibbler/ghc/ghc/issues/14451), [\#14847](http://gitlabghc.nibbler/ghc/ghc/issues/14847), [\#15142](http://gitlabghc.nibbler/ghc/ghc/issues/15142), and (to a lesser extent) [\#12088](http://gitlabghc.nibbler/ghc/ghc/issues/12088), which is all about instances.
...
##
Simplifying `getInitialKinds` ([\#15142](http://gitlabghc.nibbler/ghc/ghc/issues/15142))
...
## Trouble when fixing [\#14066](http://gitlabghc.nibbler/ghc/ghc/issues/14066)



Ticket [\#14066](http://gitlabghc.nibbler/ghc/ghc/issues/14066) requests a more robust way of tracking the scope of type
...
**Note:** This is implemented. See the commit messages in [\#9200](http://gitlabghc.nibbler/ghc/ghc/issues/9200).
...
This is somewhat simpler, it covers classes. See [comment:19:ticket:9200](http://gitlabghc.nibbler/ghc/ghc/issues/9200) for more exposition.
This change alone is enough to satisfy [\#9200](http://gitlabghc.nibbler/ghc/ghc/issues/9200).
...
my GitHub fork](https://github.com/goldfirere/ghc). It is tracked in ticket [\#9427](http://gitlabghc.nibbler/ghc/ghc/issues/9427).
...
For closed type families, these rules are *different* than the implementation today, because kind inference for closed type families today is ill-specified. See [comment:18:ticket:9200](http://gitlabghc.nibbler/ghc/ghc/issues/9200).
...
This will fail despite the CUSK for `S` because `SSyn` lacks one.  (The variation below would fix this particular example.)  I think [\#9151](http://gitlabghc.nibbler/ghc/ghc/issues/9151) is another example.
...
In [comment:5:ticket:9200](http://gitlabghc.nibbler/ghc/ghc/issues/9200), I discuss "good" polymorphism and "bad" polymorphism. This discussion, in retrospect, seems tangential at this point. It really only makes sense when discussing closed type families, which aren't at the heart of the problems here. **End Richard**
