...
The `--disable-large-address-space` disables the runtime's two-step allocator, which isn't currently supported on FreeBSD prior to 11.1 and GHC earlier than 8.6 (see [\#12695](http://gitlabghc.nibbler/ghc/ghc/issues/12695)).
...
>
>
> See [\#7472](http://gitlabghc.nibbler/ghc/ghc/issues/7472) for possible symptoms.
>
>
...
- In an extremely high-speed environment it might happen that `gmake` restarts multiple times, c.f. [\#7592](http://gitlabghc.nibbler/ghc/ghc/issues/7592).  This is because timestamp precision for the VFS layer is set to a POSIX-friendly value by default. Adjusting the `vfs.timestamp_precision` sysctl(3) variable can help.
